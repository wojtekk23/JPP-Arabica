...
typeTopDef x = case x of
  Arabica.Abs.FnDef _ type_ ident args block -> do
...
typeStmts fnType x = case x of
  ...
  Arabica.Abs.Decl _ type_ items -> do
    ...
    pure newTypeEnv
  Arabica.Abs.Ass _ ident expr -> do
    typeEnv <- ask
  ...
  Arabica.Abs.ArrAss _ ident indexExpr valExpr -> do
    typeEnv <- ask
  ...
  Arabica.Abs.SExp _ expr -> do
    typeExpr expr
    ask
  Arabica.Abs.ForTo _ ident expr1 expr2 stmt -> do
...
confirmTypes :: Arabica.Abs.Ident -> Integer -> [Arabica.Abs.AbsType] -> [Arabica.Abs.AbsType] -> Arabica.Abs.TypeCheckingMonadIO ()
...
typeExpr x = case x of
  ...
  Arabica.Abs.EArrElem _ ident expr -> do
    typeEnv <- ask
  ...
  Arabica.Abs.EApp _ ident exprs -> do
    typeEnv <- ask
  ...
  Arabica.Abs.EMul _ expr1 op expr2 -> do
  ...
  Arabica.Abs.EAdd _ expr1 _ expr2 -> do
    exprType1 <- typeExpr expr1
    case exprType1 of
      Arabica.Abs.Int -> do
        exprType2 <- typeExpr expr2
        case exprType2 of
          ...
          _ -> typeError $ unwords ["Second operand of", show x, "is not an integer"]
      _ -> typeError $ unwords ["First operand of", show x, "is not an integer"]
  Arabica.Abs.ERel _ expr1 _ expr2 -> do
    exprType1 <- typeExpr expr1
    case exprType1 of
      Arabica.Abs.Int -> do
        exprType2 <- typeExpr expr2
